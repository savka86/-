...
const client = new OpenAI({
  ...
  defaultHeaders: {
    // Эти заголовки необязательные, но помогают OpenRouter правильно подписывать приложение.
    "HTTP-Referer": process.env.OPENROUTER_SITE_URL || "https://render.com",
    "X-OpenRouter-Title": process.env.OPENROUTER_APP_NAME || "Digital Double Teacher Chat"
  }
});
...
app.post("/chat", async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Ошибка при обращении к OpenRouter. Проверьте API-ключ, баланс, лимиты и название модели."
    });
  }
});
